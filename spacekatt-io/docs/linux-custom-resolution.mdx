...
> TODO: note about setting EDID

## Failure Inspection

Now, we must inspect the initial failure again and determine its root cause.

**TODO: failure image insert**

Linux system logs and source code provide clues to the mechanism of failure for our Adafruit display.

### `dmesg` Inspection

We wish to divine why our screen is visible for a few seconds before failing to a blank screen.
...
[`drm`](#linux-drm-and-kms) is spawned early in boot and initializes [`i915`](#drmi915) (to reconcile my local Intel graphics chipset).

`i915` detects an HDMI device, `HDMI-A-1`, and reads its EDID metadata.
...
Before DRM finishes initializing, [VBE](#vesa-bios-extentions-and-uefi-gop) sets the initial video output and provides the screen with a few seconds of graphical input.
