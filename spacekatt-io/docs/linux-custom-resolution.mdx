...
To understand why a fresh Ubuntu distribution would fail to a blank screen - but only after a few seconds of successfully display at boot, we must first grapple with the basics of Ubuntu's graphics system and the seven inch Adafruit monitor itself.
